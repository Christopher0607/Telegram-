...
class Engine:
    ...
    def local_day(self) -> str:
        return (datetime.now(timezone.utc) + timedelta(hours=self.cfg.tz_offset)).strftime("%Y-%m-%d")

    def live_losses_today(self) -> int:
        """今天（按 timezone_offset_hours 的本地日期）已平仓且亏损的实盘单数。"""
        local = datetime.now(timezone.utc) + timedelta(hours=self.cfg.tz_offset)
        start = local.replace(hour=0, minute=0, second=0, microsecond=0) - timedelta(hours=self.cfg.tz_offset)
        start_ms = int(start.timestamp() * 1000)
        return sum(1 for t in self.db.closed_trades()
                   if t["mode"] == "live" and (t.get("closed_at") or 0) >= start_ms and (t.get("pnl") or 0) < 0)

    def loss_count_reason(self) -> str | None:
        limit = int(self.cfg.risk.get("daily_max_losses") or 0)
        if limit <= 0 or self.db.kv_get("loss_limit_resumed_day") == self.local_day():
            return None  # 不限制，或今天已手动 /resume
        n = self.live_losses_today()
        return f"当日亏损 {n} 单达到上限 {limit} 单" if n >= limit else None
    ...
    async def make_plan(self, act, ctx, mode, risk) -> dict:
        ...
        if mode == "live":
            ...
            r = self.loss_count_reason()
            if r:
                self.set_paused(r)
                raise Reject(f"实盘开新仓已暂停：{r}")
    ...
    async def check_daily_loss(self):
        if not (self.cfg.live_trading and self.ex.has_keys):
            return
        eq = await self.ex.equity()
        today = self.local_day()
        rec = self.db.kv_get("day_start") or {}
        if rec.get("day") != today:
            self.db.kv_set("day_start", {"day": today, "equity": eq})
            if (self.paused() or "").startswith("当日亏损"):
                self.set_paused(None)
                await self.notify("▶️ 新的一天，实盘开仓已自动恢复")
            return
        r = self.loss_count_reason()
    ...
    async def handle_command(self, text: str) -> str:
        ...
        if cmd == "/resume":
            self.set_paused(None)
            self.db.kv_set("loss_limit_resumed_day", self.local_day())  # 今天不再因亏损单数自动暂停
            return "▶️ 已恢复实盘开新仓"
        if cmd == "/closeall":
            async with self.lock:
    ...
    async def status_text(self) -> str:
        ...
        if self.ex.has_keys:
            try:
                ...
                rec = self.db.kv_get("day_start") or {}
                day = f"（今日 {(eq / rec['equity'] - 1) * 100:+.2f}%）" if rec.get("day") == self.local_day() and rec.get("equity") else ""
                lines.append(f"💰 实盘权益 {eq:.2f}U，可用 {free:.2f}U{day}")
            except Exception as e:
